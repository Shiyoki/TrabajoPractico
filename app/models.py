...
class Person(db.Model):
    ...
    license_plate = db.relationship('Vehicle', backref='plate', uselist=False)
    parking_usage = db.relationship('Usage', backref='parking_log', lazy=True)
...
class Lot(db.Model):
    ...
    vehicle_plate = db.relationship('Vehicle', backref='vehicle_lot', uselist=False)
...
class Service(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    client_id = db.relationship('Person', backref='client', uselist=False)
...
class Usage(db.Model):
    ...
    person_id_document = db.Column(db.Integer, db.ForeignKey('person.id_document'))
